...
import { apiResultFormat, pageSelection, ticketsopen } from 'src/app/core/models/models';
...
export class TicketsOpenComponent {
  ...
  public tickets: Array<ticketsopen> = [];
  country = 'India';
  isCollapsed = false;
  ...
  private getTableData(): void {
    ...
    this.data.getTicketOpen().subscribe((data: apiResultFormat) => {
      this.totalData = data.totalData;
      data.data.map((res: ticketsopen, index: number) => {
        const serialNumber = index + 1;
        if (index >= this.skip && serialNumber <= this.limit) {
          res.ticketsopen = serialNumber;
          this.tickets.push(res);
          this.serialNumberArray.push(serialNumber);
        }
      });
      this.dataSource = new MatTableDataSource<ticketsopen>(this.tickets);
