...
import { apiResultFormat, deleteaccount, pageSelection } from 'src/app/core/models/models';
...
export class DeleteAccountRequestComponent implements OnInit {
  ...
  private getTableData(): void {
    ...
    this.data.getdeleteaccount().subscribe((data: apiResultFormat) => {
      this.totalData = data.totalData;
      data.data.map((res: deleteaccount, index: number) => {
        const serialNumber = index + 1;
        if (index >= this.skip && serialNumber <= this.limit) {
          res.sNo = serialNumber;
          this.delete.push(res);
          this.serialNumberArray.push(serialNumber);
        }
      });
      this.dataSource = new MatTableDataSource<deleteaccount>(this.delete);
 ...
 // eslint-disable-next-line @typescript-eslint/no-explicit-any
  public searchData(value: any): void {
    ...
    this.delete = this.dataSource.filteredData;
  }

  public sortData(sort: Sort) {
    const data = this.delete.slice();

    if (!sort.active || sort.direction === '') {
      this.delete = data;
    } else {
      this.delete = data.sort((a, b) => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
