...
export class SalesreportComponent implements OnInit {
  ...
  public salesReport: Array<sales> = [];
    ...
    this.data.getSalesReport().subscribe((data: apiResultFormat) => {
      ...
      data.data.map((res:sales, index: number) => {
        ...
        if (index >= this.skip && serialNumber <= this.limit) {
          res.id = serialNumber;
          this.salesReport.push(res);
          this.serialNumberArray.push(serialNumber);
        }
      });
      this.dataSource = new MatTableDataSource<sales>(this.salesReport);
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
  public searchData(value: any): void {
    ...
    this.salesReport = this.dataSource.filteredData;
  }

  public sortData(sort: Sort) {
    const data = this.salesReport.slice();

    if (!sort.active || sort.direction === '') {
      this.salesReport = data;
    } else {
      this.salesReport = data.sort((a, b) => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
